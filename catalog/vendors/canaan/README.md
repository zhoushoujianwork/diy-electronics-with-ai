...
`audio_pdm.py` 示例，但示例明确按庐山派配置 GPIO，不能单凭该示例写 V3.0 接线。
现已通过 V3.0 官方工程与 TRM 独立核验音频引脚，见
[V3.0 板卡档案](../../../boards/canaan-canmv-k230-v3p0/README.md)。单颗 PDM 麦克风仍需
核对该版本按双声道组采集的行为、有效左右槽与时钟范围。
...
- [`products/canmv-k230-v3p0.yaml`](products/canmv-k230-v3p0.yaml) 已补入公开 V3.0 工程来源、
  LPDDR4 器件型号、麦克风、音频插座和 40Pin 排针设计，并链接了音频板卡档案。
  PDM 路线、电压域、复用和 MIC1 偏置/耦合网络已核验；LPDDR 数值容量、完整引脚映射、
  其余电压域与复用、供电限制和机械参数尚未完成核验。这是核验进度，不是官方资料缺失的结论。
