...
## 2026-09-21 精细像素机车电脑预览

- 把立创实战派首页的 320×108 RGB565 像素机车提取为固件与电脑工具共用的无堆
  分配渲染器。电脑端生成 3 倍整数缩放图，并提供 320×240 交互页面预览；实际点击
  已确认发动机与排气按钮均单步前进，排气图片和名称同步更新。
- 机车补充双后视镜、仪表和头灯、座椅缝线、尾灯、油箱徽记、车架高光、前刹车盘
  与卡钳、后链轮与链条、脚踏控制、发动机散热片和壳体。五种排气使用独立轮廓；
  可乐罐包含拉环、银色卷边、气泡、高光和通用 `COLA` 字标，另有 `OEM`、`AKRA`、
  `YOSH`、`OPEN` 像素铭牌。所有字标均为项目自绘的描述性非官方像素图。
- 主机 CTest 5/5 通过，新增测试覆盖画布边界保护、确定性渲染、五种排气差异和车轮
  相位差异。ESP-IDF 5.5.2 立创板构建通过，无新增编译警告；应用大小 `0xb5fc0`，
  最小应用分区仍余 29%。
