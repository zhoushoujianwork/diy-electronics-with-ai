...
int main(int argc, char **argv) {
    if(argc!=3 && argc!=4) {
        fprintf(stderr,"Usage: %s PROFILE [EXHAUST] output.wav\nProfiles:",argv[0]);
        for(unsigned i=0;i<EV_PROFILES;i++) fprintf(stderr," %s",ev_profiles[i].name);
        fprintf(stderr,"\nExhausts:");
        for(unsigned i=0;i<EV_EXHAUSTS;i++) fprintf(stderr," %s",ev_exhausts[i].name);
        fputc('\n',stderr);
        return 2;
    }
    ...
    if(argc==4) {
        snprintf(command,sizeof(command),"exhaust %s",argv[2]);
        if(ev_command(&e.control,command)!=0) return 2;
    }
    ev_control_t control=e.control;
    ev_init(&e); ev_set_control(&e,&control);
    const char *output=argv[argc-1];
    FILE *f=fopen(output,"wb"); if(!f) { perror("fopen"); return 1; }
    ...
    printf("%s/%s: 10 s, %u Hz, mono PCM16, firings=%llu -> %s\n",argv[1],
        ev_exhausts[control.exhaust].name,EV_RATE,(unsigned long long)e.firings,output);
    return 0;
}
